...
關鍵服務拆法：
- `apps/api/src/routes/*` 只負責路由 + zod 驗證 + 呼叫 service
- `apps/api/src/services/*` 是業務邏輯（products、watchlist、receipts、price-match、snapshot-diff）
- `apps/api/src/notify/*` 通知層：`Notifier` 介面 + `dispatch()` 路由器，現有 4 個實作（email / line / telegram / webpush）
- `apps/api/src/cron/daily-fetch.ts` 是排程進入點，把 services 串起來
- `apps/api/src/auth/*` 是 Google OAuth + HS256 session JWT + `requireAuth` middleware
...
- **D1 query**：用 `prepare(...).bind(...).first()` / `.all<RowType>()` / `.run()`，型別參數對齊 [`packages/shared/src/types.ts`](../packages/shared/src/types.ts) 裡的 `*Row` interface。
- **HTTP request body 驗證**：永遠用 [`packages/shared/src/schemas.ts`](../packages/shared/src/schemas.ts) 裡的 zod schema（`.parse(await c.req.json())`），不要手寫驗證。
- **TailwindCSS components**：共用 class 看 [`apps/web/src/index.css`](../apps/web/src/index.css)（`btn`、`btn-primary`、`btn-ghost`、`card`、`badge`）。預設深色主題（`<html class="dark">`、`color-scheme: dark`）。
...
pnpm --filter @costco/web run build       # 產 PWA 靜態檔到 apps/web/dist
...
封裝在 [`packages/scraper/src/client.ts`](../packages/scraper/src/client.ts)。**永遠透過 `fetchAllByCategory` / `fetchProductByCode` / `searchProducts`，不要直接 `fetch`** — 客戶端已內建 3 次指數 backoff、404→null、自訂 UA。Headers 必填 `Accept: application/json`、`User-Agent: ... CostcoTwPriceMatch/1.0 (personal use)`。

特價偵測規則（也都已寫在 [`packages/scraper/src/parser.ts`](../packages/scraper/src/parser.ts)）：
| 規則 | 邏輯 |
| --- | --- |
...
- **Worker secrets**：dev 用 [`apps/api/.dev.vars`](../apps/api/.dev.vars.example)，prod 用 `wrangler secret put`。秘密一律不入 git。完整清單在 [`RUNBOOK.md`](../RUNBOOK.md)。
...
- **Web Crypto vs Node `crypto`**：Worker runtime **不支援** Node 的 `crypto`；VAPID + AES-GCM 都用 Web Crypto API ([`apps/api/src/notify/webpush-protocol.ts`](../apps/api/src/notify/webpush-protocol.ts))。**不要 `import { ... } from 'node:crypto'`**。
...
- **`scraper/test/*.test.ts` rootDir 衝突**：scraper 的 `tsconfig.json` 沒有 `rootDir` 設定就是這個原因，不要加回去。
- **D1 datetime**：用 SQLite 內建 `datetime('now')` 與 `julianday('now') - julianday(date)`。30 天計算用 [`apps/api/src/services/products.ts`](../apps/api/src/services/products.ts) 的 `computeDays`，已處理跨午夜邊界。
- **vite-plugin-pwa**：本專案用 `injectManifest` 策略（為了 Web Push 的 custom SW），**不要切回 `generateSW`**；改 SW 邏輯改 [`apps/web/src/sw.ts`](../apps/web/src/sw.ts)。
